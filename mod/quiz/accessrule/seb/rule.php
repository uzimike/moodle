...
use mod_quiz\local\access_rule_overridable;
 ...
 */
class quizaccess_seb extends access_rule_base implements access_rule_overridable {
     ...
     * @return string
     */
    protected static function get_override_default_field(
            string $field, string $default, ?\stdClass $override, \stdClass $quiz, bool $removeprefix = false): string {
        if ($removeprefix) {
     ...
     *
     * @return array (name, title, expand)
     */
    public static function get_override_form_section_header(): array {
        return ['name' => 'seb', 'title' => get_string('seb', 'quizaccess_seb')];
    }
    ...
    public static function get_override_form_section_expand(edit_override_form $quizform): bool {
        global $DB;
        return $DB->record_exists('quizaccess_seb_override', ['overrideid' => $quizform->get_overrideid(), 'enabled' => 1]);
    }
    ...
    public static function add_override_form_fields(edit_override_form $quizform, MoodleQuickForm $mform): void {
        global $DB;
        $override = $DB->get_record('quizaccess_seb_override', ['overrideid' => $quizform->get_overrideid()]) ?: null;
        $context = $quizform->get_context();
        $quiz = $quizform->get_quiz();
        $templateoptions = settings_provider::get_template_options($quiz->cmid);

        // Enable the SEB override.
        $options = [
            0 => get_string('no'),
            1 => get_string('yes'),
        ];
        $mform->addElement('select', 'seb_enabled', get_string('enableoverride', 'quizaccess_seb'), $options);
        ...
        foreach (settings_provider::get_seb_config_elements() as $name => $type) {
            ...
            $mform->setDefault($name,
                self::get_override_default_field($name, 1, $override, $quiz));
     ...
     * @return array $errors the updated $errors array.
     */
    public static function validate_override_form_fields(
            array $errors, array $data, array $files, edit_override_form $quizform): array {
        $context = $quizform->get_context();
